...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/painel/${data.artist._id ?? data.artist.id}`);
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <div className="flex-1 flex items-center justify-center bg-gray-50 py-12">
        <section className="max-w-md w-full bg-white shadow-lg rounded-lg p-8 mx-4">
          ...
          <div className="mt-4">
            <a href="/recuperar-senha" className="text-sm text-[#1e3a8a] hover:underline">
              Esqueceu sua senha?
            </a>
          </div>
          ...
          <p className="text-center text-gray-600">
            Não tem uma conta?{" "}
            <a href="/cadastro-de-artista" className="text-[#1e3a8a] hover:underline font-semibold">
              Cadastre-se aqui
            </a>
          </p>
        </section>
