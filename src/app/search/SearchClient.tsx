  ...
  async function loadAll() {
    try {
      ...
      const [artistsRes, collectivesRes, equipmentsRes] = await Promise.all([
        fetch(`${API_ENDPOINTS.artists}?limit=1000`),
        fetch(`${API_ENDPOINTS.collectives}?limit=1000`),
        fetch(`${API_ENDPOINTS.equipments}?limit=1000`),
      ]);
      ...
      <div className="px-4 lg:px-40 py-10">
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-[#1e3a8a] mb-2">
            Encontrar Artistas e Espaços Culturais em Franca
          </h1>
